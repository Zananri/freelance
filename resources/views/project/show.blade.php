    <x-office-layout>
        ...
        <script>
            (function(){
                // Wire show/hide handlers for project-scoped Add Task modal so existing task.js logic
                // (which binds to #addTaskForm and uses ids like task_project_input / task_project_id)
                // will work when modal is opened from Project Detail page.
                try {
                    var modalEl = document.getElementById('addTaskModalProject');
                    if (!modalEl) return;

                    modalEl.addEventListener('show.bs.modal', function () {
                        try {
                            var projectIdMeta = document.querySelector('meta[name="project-id"]');
                            var currentProjectId = projectIdMeta ? projectIdMeta.getAttribute('content') : null;
                            var taskProjectIdInput = document.getElementById('task_project_id');
                            var taskProjectInput = document.getElementById('task_project_input');
                            var taskSelectedProject = document.getElementById('task_selected_project');
                            var taskProjectDropdown = document.getElementById('task_project_dropdown');
                            var taskProjectContainer = taskProjectInput ? taskProjectInput.closest('.mb-3') : null;

                            if (currentProjectId) {
                                if (taskProjectIdInput) taskProjectIdInput.value = currentProjectId;
                                if (taskProjectContainer) taskProjectContainer.style.display = 'none';
                                else {
                                    if (taskProjectInput) taskProjectInput.style.display = 'none';
                                    if (taskProjectDropdown) taskProjectDropdown.style.display = 'none';
                                    if (taskSelectedProject) taskSelectedProject.style.display = 'none';
                                }
                                if (taskProjectInput) { taskProjectInput.removeAttribute('required'); taskProjectInput.disabled = true; }

                                // Load tasks for parent selection (lightweight fallback used by task.js exists as loadProjectTasksForRelated)
                                try { if (typeof loadProjectTasksForRelated === 'function') loadProjectTasksForRelated(currentProjectId); }
                                catch(_) { }
                            }

                            // default priority and dates
                            try {
                                var prio = document.getElementById('task_priority');
                                if (prio && (!prio.value || String(prio.value).trim() === '')) prio.value = 'MEDIUM';

                                var startEl = document.getElementById('task_start_date');
                                var dueEl = document.getElementById('task_due_date');
                                var now = new Date();
                                ...
                                var dueDate = new Date(now.getTime()); dueDate.setDate(dueDate.getDate() + 2);
                                var dueStr = formatDate(dueDate);
                                if (startEl && (!startEl.value || String(startEl.value).trim() === '')) startEl.value = todayStr;
                                if (dueEl && (!dueEl.value || String(dueEl.value).trim() === '')) dueEl.value = dueStr;
                            } catch(_) {}
                        } catch(_) {}
                    });
                    ...
                    modalEl.addEventListener('hidden.bs.modal', function () {
                        try {
                            ...
                            if (imageLabel && imageClearBtn) {
                                imageLabel.style.backgroundImage = "url('" + (window.APP_URL || '') + "/asset/img/background/add-image.png')";
                            ...
                            // reset project input visibility
                            var taskProjectInput = document.getElementById('task_project_input');
                            var taskSelectedProject = document.getElementById('task_selected_project');
                            var taskProjectDropdown = document.getElementById('task_project_dropdown');
                            var taskProjectId = document.getElementById('task_project_id');
                            if (taskProjectInput) { taskProjectInput.style.display = 'block'; taskProjectInput.setAttribute('required','required'); taskProjectInput.disabled = false; taskProjectInput.value = ''; }
                            if (taskProjectDropdown) { taskProjectDropdown.style.display = 'block'; taskProjectDropdown.innerHTML = ''; }
                            if (taskSelectedProject) { taskSelectedProject.innerHTML = ''; taskSelectedProject.style.display = 'none'; }
                            if (taskProjectId) taskProjectId.value = '';

                            // reset parent
                            var taskParentInput = document.getElementById('task_parent_input');
                            var taskParentId = document.getElementById('task_parent_id');
                            var taskSelectedParent = document.getElementById('task_selected_parent');
                            var taskParentDropdown = document.getElementById('task_parent_dropdown');
                            ...
                            if (taskParentDropdown) { taskParentDropdown.innerHTML = ''; taskParentDropdown.style.display = 'none'; }
                        } catch(_) {}
                    });
                } catch(e) { console.warn('project addTask modal wiring failed', e); }
            })();
        </script>
        <script>
            // Fallback capture-phase submit handler for Add Task form specifically on Project Detail page.
            // This ensures that even if other handlers are not attached or prevented, the modal will still
            // perform a robust AJAX submit to /task/store. It mirrors logic in public/asset/js/task.js but
            // kept small and local to avoid editing the big shared file.
            (function(){
                try {
                    var addTaskForm = document.getElementById('addTaskForm');
                    ...
                    function syncQuillIfPresent(quill, taId) {
                        try { if (quill && quill.root && document.getElementById(taId)) {
                            document.getElementById(taId).value = quill.root.innerHTML || '';
                        }} catch(_) {}
                    }
                    ...
                    addTaskForm.addEventListener('submit', function(e){
                        try {
                            ...
                            if (addTaskModalEl && !addTaskModalEl.classList.contains('show') && !addTaskModalEl.classList.contains('modal')) {
                            ...
                            // Sync Quill editors
                            try { if (window.__quillTaskAdd) syncQuillIfPresent(window.__quillTaskAdd, 'task_description'); } catch(_){}
                            ...
                            try {
                                ...
                                if (!Array.isArray(execArr) || execArr.length === 0) {
                                    try { if (typeof showFloatingAlert === 'function') showFloatingAlert('Please select at least one executor.', 'warning', 2500); else alert('Please select at least one executor.'); } catch(_) { alert('Please select at least one executor.'); }
                                    return;
                                }
                            } catch(_) {}
                            ...
                            var submitBtn = addTaskForm.querySelector("button[type='submit']");
                            ...
                            try {
                                var parentSel = document.getElementById('task_parent_id');
                                if (parentSel) {
                            ...
                            try {
                                ...
                                if (imgEl && imgEl.files && imgEl.files[0]) fd.set('image', imgEl.files[0]);
                            } catch(_) {}

                            // Append reference files if any (input id task_reference_files) and also use selectedFiles if present globally
                            try {
                                var refInput = document.getElementById('task_reference_files');
                                if (refInput && refInput.files && refInput.files.length) {
                                    for (var i=0;i<refInput.files.length;i++) fd.append('reference_files[]', refInput.files[i]);
                                }
                            } catch(_) {}
                            ...
                            fetch((window.APP_URL || '') + '/task/store', {
                                ...
                                if (!res.ok) return res.json().then(function(json){ throw { status: res.status, body: json }; });
                                return res.json();
                            }).then(function(json){
                                try { if (loader) loader.classList.add('d-none'); } catch(_) {}
                                if (submitBtn) submitBtn.disabled = false;
                                try { if (typeof showFloatingAlert === 'function') showFloatingAlert(json.message || 'Task added successfully!', 'success'); } catch(_) {}

                                // close modal after short delay and reload
                                setTimeout(function(){
                                    try { if (addTaskModalEl && addTaskModalEl.dataset) addTaskModalEl.dataset.allowProgrammaticClose = '1'; } catch(_){}
                                    try { var m = bootstrap.Modal.getInstance(addTaskModalEl); if (m) m.hide(); } catch(_){}
                                ...
                                try {
                                    if (err && err.body) {
                                        if (err.body.errors) msg = Object.values(err.body.errors).flat().join('\n');
                                        else if (err.body.message) msg = err.body.message;
                                    }
                                } catch(_) {}
                                try { if (typeof showFloatingAlert === 'function') showFloatingAlert(msg, 'danger'); else alert(msg); } catch(_) { alert(msg); }
                            });
                        } catch(e){
                            try { if (loader) loader.classList.add('d-none'); } catch(_) {}
                ...
                } catch(e) { console.warn('AddTask fallback install failed', e); }
            })();
        </script>
