document.addEventListener("DOMContentLoaded", function () {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    setTimeout(() => {
                        ...
                        // Show success alert
                        let alertContainer = document.querySelector("#addTaskModal .modal-dialog .alert-container");
                        if (!alertContainer) {
                            alertContainer = document.createElement("div");
                            alertContainer.className = "alert-container mt-2";
                            alertContainer.style.width = "100%";
                            document.querySelector("#addTaskModal .modal-dialog").appendChild(alertContainer);
                        }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    setTimeout(() => {
                        ...
                        let alertContainer = document.querySelector("#editTaskModal .modal-dialog .alert-container");
                        if (!alertContainer) {
                            alertContainer = document.createElement("div");
                            alertContainer.className = "alert-container mt-2";
                            alertContainer.style.width = "100%";
                            document.querySelector("#editTaskModal .modal-dialog").appendChild(alertContainer);
                        }
                        alertContainer.innerHTML = `<div class="alert alert-success" role="alert">${data.message || "Task updated successfully!"}</div>`;
                        alertContainer.style.display = "block";

                        // Reset form and preview (same as Add Task)
                        editTaskForm.reset();
                        const editImageLabel = document.getElementById("editTaskImageLabel");
                        const editImageClearBtn = document.getElementById("editTaskImageClearBtn");
                        if (editImageLabel) {
    ...
    const editTaskImageClearBtn = document.getElementById("editTaskImageClearBtn");

    if (editTaskImageInput && editTaskImageLabel && editTaskImageClearBtn) {
        setupImageInput(editTaskImageInput, editTaskImageLabel, editTaskImageClearBtn);
    }
    ...
    if (editTaskModalElement) {
        editTaskModalElement.addEventListener('hidden.bs.modal', function () {
            ...
            $('#editTaskImageLabel').css('background-image', "url('" + appUrl + "/asset/img/background/add-image.png')");
            ...
            window.clearSelectedExecutorsEdit && window.clearSelectedExecutorsEdit();
    ...
    function createTaskCard(task) {
        ...
        return `
            <div class="custom-card mb-3 rounded-4 position-relative" data-task-id="${task.id}">
    ...
    function setupTaskDropdownListeners() {
        ...
        document.addEventListener('click', function (e) {
            if (e.target && e.target.classList.contains('dropdown-item')) {
                ...
                const executorDropdown = e.target.closest('#executor_dropdown, #edit_executor_dropdown');
    ...
    function handleTaskDetail(taskId) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#taskDetailImage').attr('src', data.image ? appUrl + '/file/task/' + data.image : appUrl + '/asset/img/background/add-image.png');
                $('#taskDetailTitle').text(data.title || '');
                $('#taskDetailDescription').text(data.description || '');
                
                // Department and Division from project
                $('#taskDetailDepartment').text(data.project && data.project.department ? data.project.department : '');
                $('#taskDetailDivision').text(data.project && data.project.division ? data.project.division : '');
                $('#taskDetailProject').text(data.project ? data.project.title : '');
                ...
                if (data.reference_url) {
                    $('#taskDetailReferenceUrl').attr('href', data.reference_url).text(data.reference_url).show();
                } else {
                ...
                if (data.executors && data.executors.length > 0) {
                    const executorNames = data.executors.map(ex => ex.name).join(', ');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                loadProjectsForEdit(function() {
                    ...
                    // Reset image preview
                    if (data.image) {
                        $('#editTaskImageLabel').css('background-image', 'url(' + appUrl + '/file/task/' + data.image + ')');
                        $('#editTaskImageLabel').addClass('has-image');
                        $('#editTaskImageLabel').css('background-size', 'cover');
                    ...
                    if (data.executors) {
                        ...
                        window.setSelectedExecutorsEdit && window.setSelectedExecutorsEdit(executors);
                    }
    ...
    function loadProjectsForEdit(callback) {
        const editProjectSelect = document.getElementById("edit_task_project_id");
        ...
        confirmDeleteBtn.onclick = function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    alertContainer.className = 'alert alert-success d-flex align-items-center task-delete-alert';
