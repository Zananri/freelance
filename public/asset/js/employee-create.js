document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadDivisions(departmentId) {
        ...
        fetch(`/divisions?department_id=${departmentId}`, { headers: { 'Accept': 'application/json' } })
            .then(res => res.json())
            .then(data => {
                ...
                jobSelect.innerHTML = '<option value="" disabled selected>Select Job</option>'; // reset job
            });
    }
    ...
    function loadJobs(divisionId) {
        ...
        fetch(`/jobs?division_id=${divisionId}`, { headers: { 'Accept': 'application/json' } })
            .then(res => res.json())
            .then(data => {
                let options = '<option value="" disabled selected>Select Job</option>';
                (data.data || []).forEach(job => {
                    options += `<option value="${job.id}">${job.job_name || job.name}</option>`;
                });
            ...
            fetch('/employees', {
                ...
                .then(({ status, body }) => {
                    if (status === 422) {
                        ...
                        formAlert.innerHTML = '<div class="alert alert-success">Employee created successfully.</div>';
                        ...
                        formAlert.innerHTML = '<div class="alert alert-danger">Failed to create employee.</div>';
                    }
                })
                .catch(() => {
                    formAlert.innerHTML = '<div class="alert alert-danger">Failed to create employee.</div>';
