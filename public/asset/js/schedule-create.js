document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', e=>{
        ...
        fetch(appUrl + '/schedules/create', { method:'POST', headers:{ 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: fd })
            ...
            .then(({ok, body})=>{ if(loader) loader.classList.add('d-none'); if(submitBtn) submitBtn.disabled=false; if(!ok || body.code!==200){ const msg = (body && (body.message || 'Failed to create schedule')) || 'Failed.'; showScheduleAlert(msg,'danger'); return; } showScheduleAlert(body.message || 'Schedule created successfully','success'); form.reset(); selectedFiles=[]; displaySelectedFiles(); try { document.getElementById('scheduleImageLabel').style.backgroundImage = ''; document.getElementById('scheduleImageLabel').classList.remove('has-image'); document.getElementById('scheduleImageClearBtn').classList.add('d-none'); } catch(_){}; try { const modalEl = document.getElementById('scheduleCreateModal'); if (modalEl) { const modalInstance = bootstrap.Modal.getOrCreateInstance(modalEl); modalInstance.hide(); } } catch(_) {} try { if (typeof window.refreshScheduleList === 'function') { window.refreshScheduleList(); } else { // As a fallback, refetch using the local function if available
                    if (typeof fetchScheduleData === 'function') fetchScheduleData(1, '', '');
                } } catch(_) {} })
